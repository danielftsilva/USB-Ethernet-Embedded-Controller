...
The **USB 3.2 Gen 1 to Gigabit Ethernet Adapter** is a hardware design developed to provide a wired Ethernet interface to a computer through a standard USB Type-A port.
...
The objective of the project is to design a standalone adapter that connects directly to a computer through USB Type-A and converts the USB interface into a standard **10/100/1000BASE-T Ethernet interface**, allowing an external Ethernet cable to be connected to the adapter and providing network connectivity to the host computer exclusively through the designed hardware.
...
- **Kycon KUSBX-AS1N9-2-BL30** — USB 3.2 Gen 1 Type-A receptacle
...
The board includes a dedicated power-conversion stage to generate the required 3.3 V supply, together with local decoupling and filtering for the main devices.
